...
// Import skeleton loader functions
import {
  showSkeletonLoader,
  hideSkeletonLoader,
...
async function initializeApp() {
  ...
  // Show skeleton loader for pages that need it
  const listingsContainer = document.getElementById("listings-container");
  if (listingsContainer) {
    showSkeletonLoader(listingsContainer); // Show skeleton while fetching listings
  }
  ...
  // Fetch data (for example listings) and hide the skeleton loader once data is fetched
  if (listingsContainer) {
    fetch("/api/listings") // Replace with your actual API endpoint
      .then((response) => response.json())
      .then((data) => {
        hideSkeletonLoader(listingsContainer, data);
      })
      .catch((error) => {
